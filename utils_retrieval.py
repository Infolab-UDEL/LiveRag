...
class BGEReranker:
    def __init__(self, cuda_device: str = "cuda:1", model_name: str = "BAAI/bge-reranker-v2-m3", cache_dir: str = "/data_hdd/damian/llm"):
    ...
    def rerank(self, user_query: str, documents: List[str], top_n: int = None): #-> Tuple[List[int], List[float]]
        ...
        with torch.no_grad():
            ...
            ranking_list = [index for index, _ in indexed_scores]
            score_list = [score.item() for _, score in indexed_scores]

            if top_n is not None:
                return ranking_list[:top_n], score_list[:top_n]
            return ranking_list, score_list
...
def query_pinecone(query: str, top_k: int = 10, namespace: str = PINECONE_NAMESPACE) -> dict:
    ...
    results = index.query(
        vector=embed_query(query), top_k=top_k, include_values=False, namespace=namespace, include_metadata=True
    )
...
def batch_query_pinecone(
    queries: list[str], top_k: int = 10, namespace: str = PINECONE_NAMESPACE, n_parallel: int = 10
) -> list[dict]:
    ...
    results = pool.map(
        lambda x: index.query(vector=x, top_k=top_k, include_values=False, namespace=namespace, include_metadata=True),
        embeds,
    )
    return results
...
def query_opensearch_by_uuid(query: str, top_k: int = 10) -> dict:
    """Query an OpenSearch index and return the results."""
    client = get_client()
    results = client.search(index=OPENSEARCH_INDEX_NAME, body={"query": {"match": {"doc_id": query}}, "size": top_k})
    return results
